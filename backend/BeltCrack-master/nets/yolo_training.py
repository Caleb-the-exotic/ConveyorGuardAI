...
class YOLOLoss(nn.Module):    
    ...
    def get_output_and_grid(self, output, k, stride):
        ...
        output              = output.flatten(start_dim=2).permute(0, 2, 1)   # output [6, 32, 32]
        ################################
        ## output[...,2].shape   [6,32,2]

        # print("Grid", output.shape,  output[...,:2].shape  , grid.type_as(output).shape )
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        # mmdet 
        # loss_obj    = (VarifocalLoss(obj_preds.view(-1,1),obj_targets)).sum()
        # loss_cls    = (VarifocalLoss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()
