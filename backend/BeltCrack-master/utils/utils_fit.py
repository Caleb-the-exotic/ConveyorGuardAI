...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            #   前向传播
            # ----------------------#
            # heatmap=cv2.applyColorMap(images[0].permute(1,2,0).cpu().numpy().astype(np.uint8),cv2.COLORMAP_JET)
            # cv2.imwrite('test.jpg', heatmap)
    ...
    if local_rank == 0:
        ...
        if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
            torch.save(save_state_dict, os.path.join(save_dir, "ep%04d-loss%.4f-val_loss%.4f.pth" % (epoch + 1, loss / epoch_step, val_loss / epoch_step_val)))
