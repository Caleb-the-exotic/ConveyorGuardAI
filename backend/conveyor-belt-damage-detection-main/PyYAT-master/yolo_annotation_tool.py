...
def pre_annotate(frame, all_labels):
	# No pre-annotation for manual damage annotation
	return []


def convert_list_to_str(lst):
    return str(lst).translate(str.maketrans('', '', '[],\''))
...
def save_annotation(frame, labels, image_name):
	output_dir = output_folder
	base_name = os.path.splitext(os.path.basename(image_name))[0]
	cv2.imwrite(os.path.join(output_dir, base_name + ".jpg"), frame)
	file=open(os.path.join(output_dir, base_name + ".txt"),'w')
	for items in labels:
...
def draw_boxes(frame, yolo_boxes, frame_orig, image_path):
	...
	while keep_running != 0:
		...
		if ch == ord('s'):
			...
			save_annotation(frame_orig, yolo_labels, image_path)
			...
			try:
				idx = all_labels.index(current_label)
			except ValueError:
				idx = 0
			idx = (idx + 1) % len(all_labels)
...
for im in range(len(list_of_images)):
	if annotation_index >= len(list_of_images):
		print("All images processed!")
		break
		
	current_image_path = list_of_images[annotation_index]
	...
	# show_image(frame)
	draw_boxes(frame, yolo_boxes, frame_orig, current_image_path)
